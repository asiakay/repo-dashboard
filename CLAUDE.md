...
OKR/Pipeline micro-task text (Today tab OKR rows, OKR Progress task lists and "today" list, Pipeline cards) and work-item text (Today, Capacity, Active Work, Agent Tasks, Priority) is rendered with `taskLink(text, id, kind)` in `public/js/app.js`, where `kind` is `"tasks"` or `"work"`. Clicking one opens `#task-search-modal` (a native `<dialog>` in `public/index.html`), which searches the data the page has already loaded (OKR tasks, OKRs, work items, repos, resources). It runs fully client-side with no API call. Matching is per-word with stopwords removed and prefix matching (≥4 chars). The clicked task is left out of its own results. Clicking a result switches to the relevant tab (clearing a conflicting Pipeline OKR filter, or switching Active Work to "Completed" for a done item), or opens the repo on GitHub. Styles live under "Task search" in `public/styles.css`. The focus strip keeps its own click behavior and is not linked.
